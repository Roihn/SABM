...
class Human(Agent):
    ...
    def form_current_state_stage1(self, num_agents_around, dist_to_nearest_exit):
        ...
        prompt += dedent(f"""\
            ...
            Please tell me your feeling about the situation around you in one sentence showing if you are panic or not.
            """)
        return dedent(prompt)
    
    def form_current_state_stage2(self, panic_level_str, data):
        prompt = dedent(f"""Now you feel: "{panic_level_str}".\nHere shows you the distances to different exits and the number of people you can see towards those exits.\n""")
        ...
        prompt += dedent(f"""Please tell me briefly how will you evaluate the two aspects of each exit based on your feeling in one sentence. Please give 3 sentences for each exit (around 15 words).""")

        return dedent(prompt)
    
    def form_current_state_stage3(self, stage1_feeling, evaluation):
        prompt = dedent(f"""Now you feel: \"{stage1_feeling}\".\nThere are 3 exits in this room. Base on the current situation, your personal feeling on each exit are:\n
{evaluation}""")
        target_exit = None
        if self.target_exit:
