...
class EscapeSociety(Society):
    def __init__(self, name, num_humans:int, agent_chat_range:int, width:int, height:int, exit_width:int=3, seed=0, need_obstacle=False, random_agent=True, is_panic=True, update_rounds=5, model="gpt-4-0314", api_key=None) -> None:
        ...
        if random_agent:
            self.output_path = f"output/emergency_evacuation/task4/need_obstacle_{self.need_obstacle}/{self.num_humans}humans/is_panic_{self.is_panic}_seed{self.seed}"
        else:
            self.num_humans = 66 * 4
            self.output_path = f"output/emergency_evacuation/task4/need_obstacle_{self.need_obstacle}/fixed_agent/is_panic_{self.is_panic}_seed{self.seed}"
    ...
    def add_human_reproduce(self, random_agent=True):
        ...
        for human_count in range(self.num_humans):
            # capability = self.rng.integers(0, 2)
            # # capability = 2
            # mental = self.rng.integers(0, 2)
    ...
    def step(self):
        ...
        for agent in gpt_human_list:
            ...
            agent.communicate_sentence = resp_communicate
            communicate_list.append((agent.id, agent.cur_pos, resp_communicate))
            agent.prompt_communicate = prompt_communicate
        ...
        for agent in gpt_human_list:
            ...
            prompt_stage1 = agent.form_current_state_stage1(agent.persona, num_agents_around, dist_to_nearest_exit)
            ...
            if flag is None:
                ...
                prompt_stage2 = agent.form_current_state_stage2(agent.persona, panic_index_str, exit_data)
            ...
            communicate_samples = n_closest_messages(agent, communicate_list, threshold_dist=5)
            prompt_stage3 = agent.form_current_state_stage3(agent.persona, panic_index_str, resp2, communicate_samples)
            ...
            agent_state = {
                ...
                'prompt_communicate': agent.prompt_communicate,
                'response_communicate': agent.communicate_sentence,
            }
            ...
            for agent in agents:
                ...
                if agent.state_history[-1]['round'] == self.round:
                    ...
                    state = {
                        ...
                        'prompt3': None,
                        'response3': None,
                        'prompt_communicate': None,
                        'response_communicate': None,
        ...
        for i in range(len(grid)):
            for j in range(len(grid[0])):
                if grid[i][j] is None and (i,j) not in [exit.pos for exit in self.exit_list]:
                    ...
                    plt.scatter(j+0.5, i+0.5, c=colors[grid[i][j].mental * 2 + grid[i][j].capability])
