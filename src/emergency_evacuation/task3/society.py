...
class EscapeSociety(Society):
    def __init__(self, name, num_humans:int, agent_chat_range:int, width:int, height:int, exit_width:int=3, seed=0, need_obstacle=False, random_agent=True, is_panic=True, update_rounds=5, model="gpt-4-0314", api_key=None) -> None:
        ...
        if random_agent:
            self.output_path = f"output/emergency_evacuation/task3/need_obstacle_{self.need_obstacle}/{self.num_humans}humans/is_panic_{self.is_panic}_seed{self.seed}"
        else:
            self.num_humans = 66 * 4
            self.output_path = f"output/emergency_evacuation/task3/need_obstacle_{self.need_obstacle}/fixed_agent/is_panic_{self.is_panic}_seed{self.seed}"
    ...
    def step(self):
        ...
        for agent in gpt_human_list:
            ...
            prompt_stage3 = agent.form_current_state_stage3(agent.persona, panic_index_str, resp2)
