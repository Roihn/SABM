...
class EscapeSociety(Society):
    def __init__(self, name, num_humans:int, agent_chat_range:int, width:int, height:int, exit_width:int=3, seed=0, need_obstacle=False, random_agent=True, is_panic=True, update_rounds=5, model="gpt-4-0314", api_key=None) -> None:
        ...
        if random_agent:
            self.output_path = f"output/emergency_evacuation/task1/need_obstacle_{self.need_obstacle}/{self.num_humans}humans/is_panic_{self.is_panic}_seed{self.seed}"
        else:
            self.num_humans = 66 * 4
            self.output_path = f"output/emergency_evacuation/task1/need_obstacle_{self.need_obstacle}/fixed_agent/is_panic_{self.is_panic}_seed{self.seed}"
    ...
    def generate_env_prompt(self):
        prompt = f"""\
            You are now the agent in the room with fire. You need to escape from the room as fast as possible. The room has a size of 33 * 33.
            There are {len(self.exit_list)} exits in the room. The exits are located at the left, bottom and right of the room."""
        if self.is_panic:
            prompt += " You are in a panic state.\n"
        else:
            prompt += " You are in a normal state.\n"
        prompt += """\
            ...
            It is important to figure out to what extent you prioritize the three aspects.
            The following shows the previous rounds of the simulation."""
    ...
    def words_to_values(self, words, beta):
    # Define the mapping from words to value ranges
        word_to_range = {
            'minimal': (0.0, 2.0),
            'mild': (2.0, 4.0),
            'moderate': (4.0, 6.0),
        ...
        values = []
        # Convert each word to a random value within the corresponding range
        for word in words:
            value_range = word_to_range.get(word)
            if value_range is None:
                raise f"Invalid word '{word}'. Only these words are allowed: {list(word_to_range.keys())}"
    ...
    def get_next_action(self, agent: Human, visible_grids: List[Tuple[int, int]], target_exit: Tuple[int, int]) -> Tuple[int, int]:
        opt_action = None
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                k = 1
                J = (math.pow(agent.alpha4, agent.beta1) * dist + math.pow(agent.alpha5, agent.beta2) * P_ij + math.pow(agent.alpha6, agent.beta3) * Q_ij) / (agent.alpha4 + agent.alpha5 + agent.alpha6)
                if J < min_ij:
        ...
        return opt_action, suboptimal_action
    ...
    def step(self):
        ...
        # Update the beta values of chosen agents with gpt
        for agent in gpt_human_list:
            ...
            state_history_prompt = agent.get_state_history()
            current_prompt = agent.form_current_state(self.round, num_agents_around, dist_to_nearest_exit)
            prompt = f"{self.env_prompt}\n {state_history_prompt}\n {current_prompt}"

            resp_words, resp = agent.communicate(prompt)
            if resp_words is None:
                if agent.beta1 is None:
                    raise Exception("Agent is not well initializied.")
            else:
                values = self.words_to_values(resp_words, beta)

                agent.beta1 = values[0] * beta
                # agent.beta2 = 1 / (values[1] * beta)
                # agent.beta3 = 1 / (values[2] * beta)
            ...
            agent_state = {
                ...
                'beta2_coef': values[1],
                'beta3_coef': values[2],
                'beta1_value': agent.beta1,
                'beta2_value': agent.beta2,
                'beta3_value': agent.beta3,
                'num_agents_around': num_agents_around,
                'dist_to_nearest_exit': dist_to_nearest_exit,
                'response': resp,
            }
            agent.update_state_history(agent_state)
            
        # Normal process: first decide the exit, then decide the action
        for agent in self.cur_human_list:
            is_normal, Y, beta0 = self.set_agent_status(agent)
            if not is_normal:
                # agent is overtaken or dead
            ...
            for exit in self.exit_list:
                ...
                ij = k * (math.pow(agent.alpha1, agent.beta1) * dist + math.pow(agent.alpha2, agent.beta2) * P_ij + math.pow(agent.alpha3, agent.beta3) * Q_ij) / (agent.alpha1 + agent.alpha2 + agent.alpha3)
                if ij < min_ij:
                    min_ij = ij
                    target_exit = exit
        ...
        for exit in agent_target_dict:
            ...
            for agent in agents:
                ...
                if agent.cur_pos in visible_grids:
                    visible_grids.remove(agent.cur_pos)
                opt_action, suboptimal_action = self.get_next_action(agent, visible_grids, target_exit)
